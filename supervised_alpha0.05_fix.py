...
for rep1 in range(40):
    ...
    for rep in range(100):
        ...
        for i in range(len(s0)):
            if s0[i]<0:
                N.append(30)
                X.append(np.random.uniform(-0.5,0.5,30)) #generate X
            else:
                N.append(30) #generate N
                X.append(np.random.uniform(-0.5,0.5,30))
        Y=[]
        for j in range(K):
        ...
        for i in range(len(r)):
            ...
            for j in range(K):
                #if N[j]==40:
                    #if np.sqrt(N[j])*abs(np.mean(mat[j])-0)/np.std(mat[j])<=2:
            ...
            mat_new1=[] #HCP: only used for random samples
            mat_new12=[]
            for j in range(K):
            ...
            if rk<round((sum(N)//2)*0.05):
                validate2[i]=1


            mat_new41=[] #Dunn et al
            for j in range(K-1):
